...
import argparse
from utils import (print_time_stamp, reset_workpath, save_history, save_log_confusion_matrix, seed_everything, save_metrics)
...
def dataFirstLoad():
    ...
    for i in range(subNum):
        ...
        for j in range(len(label_csv.label1)):
            ...
            data_trial = data_trial[:, :int(int(data_trial.shape[1] / srate / CFG.windowStep) * srate * CFG.windowStep)]
            sample = np.arange(winlen, data_trial.shape[1] / srate + 1e-8, CFG.windowStep)
            ...
            for data_i in range(len(sample)):
                seg_data = data_trial[:, (sample[data_i] - winlen * srate):sample[data_i]]
                preproc_data = preproc_seq(seg_data)
                data_tmp[data_i] = preproc_data
            np.save(os.path.join('processData', f'sub{i + 1}', f'{label_csv.label1[j]}.npy'), data_tmp)
            sample_num = sample_num + len(sample)
        sub_df = pd.concat([sub_df, pd.DataFrame({'sub': [i + 1], 'num': [sample_num]})], axis=0, ignore_index=True)
...
def dataSecondLoad(sub_train, dataType):
    ...
    for j in load_subj_bar:
        for k in range(len(label_csv.label1)):
            rawdata_tmp = np.load(os.path.join('processData', f'sub{subs[j]}', f'{label_csv.label1[k]}.npy'))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='BCI22_EMOTION')
    ...
    os.environ['CUDA_VISIBLE_DEVICES'] = str(CFG.device) if isinstance(CFG.device, int) else ','.join(str(_) for _ in CFG.device)
    ...
    for i in range(start_kfold, CFG.n_fold):
        sub_test = sub_per[i * int(CFG.all_batch / CFG.n_fold):(i + 1) * int(CFG.all_batch / CFG.n_fold)].reset_index(
            drop=True)
        sub_train_valid = sub_per[~sub_per['sub'].isin(sub_test['sub'])].reset_index(drop=True)
        sub_valid = sub_train_valid.sample(frac=0.2, random_state=CFG.seed, ignore_index=True)
        sub_train = sub_train_valid[~sub_train_valid['sub'].isin(sub_valid['sub'])].reset_index(drop=True)
